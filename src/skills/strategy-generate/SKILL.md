...
- **extra_fields**: optional daily valuation fields (`pe`, `pb`, `roe`, etc.)
- **fundamental_fields**: optional PIT-safe statement fields for financial pre-filtering
- **Data format**: `open, high, low, close, volume` + `DatetimeIndex`
...
- `source`: `"auto"` (recommended, auto-select by code format) / `"tushare"` / `"akshare"` / `"baostock"` / `"tencent"` / `"mootdx"` / `"eastmoney"`
  - `"auto"` automatically routes A-share codes to the best available source
...
- `extra_fields`: China A-shares can use values such as `["pe", "pb", "roe"]`
