---
name: shadow-account
description: "Shadow Account — 从用户交割单提炼盈利模式（3-5 条人话规则）→ A 股回测 → 差值归因 → 8-section PDF 报告。叙事：你的影子，没有情绪噪音。"
category: analysis
---
...
2. `run_shadow_backtest(shadow_id=..., journal_path=...)`
   - 返回 A 股回测指标 + `delta_pnl` + attribution breakdown
   - 默认 A 股市场回测
...
### 回测矩阵
- `per_market`：A 股市场的 Sharpe/年化/最大回撤
- `combined`：合并池表现
